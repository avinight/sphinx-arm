...
def resolve_data_file(subdir, filename):
    data_dir = resolve_data_dir(subdir)
...
csv_file = resolve_data_file("data-skew", "read_latency.csv")
data1 = pd.read_csv(csv_file) if os.path.exists(csv_file) else None
...
if data1 is None:
    ax1.text(
        0.5,
        0.5,
        "Missing data-skew/read_latency.csv",
        transform=ax1.transAxes,
        ha="center",
        va="center",
        fontsize=font_ampl * 16,
    )
    ax1.set_axis_off()
else:
...
# ==============================
data2_path = resolve_data_file("data-mt", "throughput_vs_threads_ssd.csv")
data3_path = resolve_data_file("data-mt", "throughput_vs_threads_4ReserveBitsssd.csv")
data2 = pd.read_csv(data2_path) if os.path.exists(data2_path) else None
data3 = pd.read_csv(data3_path) if os.path.exists(data3_path) else None
...
plt.savefig(os.path.join(script_dir, "combined_diagrams.svg"))
