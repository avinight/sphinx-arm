...
for col in range(6):
    ...
    axes_mem[col].set_xlabel(f"({alphbets[col]}) #entries", fontsize=font_ampl * 24)
    axes_mem[col].spines[["top", "right"]].set_visible(False)
    axes_mem[col].xaxis.set_label_coords(0.5, -0.15)

lines, labels = axes_mem[1].get_legend_handles_labels()
fig_mem.legend(
    lines, labels, loc="upper center", ncol=7, fontsize=font_ampl * 26,
